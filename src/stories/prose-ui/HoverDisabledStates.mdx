...
<div style={{ display: 'grid', gap: '16px', marginTop: '24px', marginBottom: '32px' }}>
  ...
  <div style={{ 
    ...
    <strong style={{ color: '#664B16' }}>Weight ≥ 700 and &lt; 900</strong>
    <p style={{ margin: '8px 0 0 0', color: '#333' }}>
  ...
  <div style={{ 
    ...
    <strong style={{ color: '#471167' }}>Weight ≥ 900</strong>
    <p style={{ margin: '8px 0 0 0', color: '#333' }}>
      <strong>No mix</strong> — Use the <strong>previous color</strong> from the palette
    </p>
  </div>
</div>
...
When multiple elements with the same color are stacked (e.g., card → row → icon):

1. **First element** → Default hover color
2. **Second element** → Next hover color from palette
3. **Third element** → Next hover color from palette
4. And so on...

---

## 🎪 Interactive Palette Demonstrations
...
export const ColorPalette = ({ name, colors }) => {
  ...
  const computeHoverColor = (weight) => {
    ...
    if (numWeight < 700) {
      return idx < weights.length - 1 ? colors[weights[idx + 1]] : colors[weight];
    } else if (numWeight >= 900) {
      return idx > 0 ? colors[weights[idx - 1]] : colors[weight];
    } else {
      return idx > 0 ? colors[weights[idx - 1]] : colors[weight];
    }
  };
          ...
          return (
            ...
            >
              ...
              <div style={{ textAlign: 'center', fontSize: '12px' }}>
                ...
                <div style={{ fontSize: '10px', color: isHovered ? '#4485F1' : 'transparent', marginTop: '4px', height: '14px' }}>
                  Hover: {hoverColor}
                </div>
              </div>
            </div>
...
<div style={{ 
  ...
  💡 <strong>Try it!</strong> Hover over any color swatch above to see its computed hover state in action.
</div>
...
<div style={{ 
  ...
  <h3 style={{ margin: '0 0 12px 0', color: '#166534' }}>✨ Result</h3>
